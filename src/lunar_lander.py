# Original source code: https://github.com/Farama-Foundation/Gymnasium/blob/main/gymnasium/envs/box2d/lunar_lander.py
# License: MIT
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def reset(
        ...
        *,
        ...
        # If we are close to the bounds, then clamp/limit the initial force in
        # the x direction
        INITIAL_RANDOM_L_VEL_X = -INITIAL_RANDOM if initial_x > (VIEWPORT_W / SCALE) / 6 else 0
        INITIAL_RANDOM_R_VEL_X = INITIAL_RANDOM if initial_x < (VIEWPORT_W / SCALE) - ((VIEWPORT_W / SCALE) / 6) else 0

        # Apply the initial random impulse to the lander
        self.lander.ApplyForceToCenter(
            (
                self.np_random.uniform(INITIAL_RANDOM_L_VEL_X, INITIAL_RANDOM_R_VEL_X),
